...
Button::Button() : ControlBaseExtended(100, 100, 100, 100, 1) {
	hit = pressed = false;
	this->buttonLabel = new Label(X + ((Width - (6 * strlen("Default string"))) / 2), (Y + Height - (Height - 12) / 2), (Z + 1), "Default string", 0, 0, 0);
}
...
: ControlBaseExtended(locX, locY, width, height, z) {
	hit = pressed = false;
	this->buttonLabel = new Label(X + ((Width - (6 * text.length())) / 2), (Y + Height - (Height - 12) / 2), (Z + 1), text, 0, 0, 0);
}
